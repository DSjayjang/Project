...
class Net_2d(nn.Module):
    def __init__(self, dim_in, dim_2d_desc, dim_3d_desc, 
        ...
        # self.W = nn.Parameter(torch.randn(dim_graph_emb, dim_2d_desc))
        # self.fc1 = nn.Linear(dim_2d_desc, dim_out_fc1)
        self.W = nn.Parameter(torch.randn(dim_2d_desc, dim_graph_emb))
    ...
    def forward(self, g, desc_2d, desc_3d):
        ...
        # bilinear form
        # A = hg @ self.W     # (B, d_g) @ (d_g, d_2d)
        # hg = A * desc_2d    # (B, d_2d) * (B, d_2d)
        A = desc_2d @ self.W     # (B, d_2d) @ (d_2d, d_g)
        hg = A * hg    # (B, g) * (B, g)
