...
class Net(nn.Module):
    def __init__(
        ...
        # MLP head
        self.fc1 = nn.Linear((d_k+1)*(d_k+1), dim_out_fc1)
    ...
    def forward(self, g, desc_2d, desc_3d):
        ...
        # bidirectional cross-attn -> two vectors
        ...
        tensor_g_vec = torch.cat((g_vec, ones), dim=1)            # (B, d_g+1)
        tensor_d_vec = torch.cat((d_vec, ones), dim=1)   # (B, d_k+1)

        fusion_tensor = torch.bmm(tensor_g_vec.unsqueeze(2), tensor_d_vec.unsqueeze(1))  # (B, d_g+1, d_k+1)
        final = fusion_tensor.view(B, -1)                                               # (B, (d_g+1)*(d_k+1))
