...
def get_parser():
    ...
    parser.add_argument('--loss', type=str, default='cats', choices=['cats','agcn'])

    return parser

class Cats(nn.Module):
    def __init__(self, backbone, C=400, d=256, H=4, N=18, T=300):
        super().__init__()
...
class Processor():
    ...
    def load_model(self):
        ...
        if self.arg.loss == 'cats':
            print('CATS-GCN...')
            self.model = Cats(self.model).cuda(output_device)
            self.loss = CATSUnifiedLoss(
                num_classes = self.arg.model_args.get('num_class')
            ).cuda(output_device)
        elif self.arg.loss == 'agcn':
            print('2s-AGCN...')
        ...
        if self.arg.weights:
            ...
            for k, v in weights.items():
                if k.startswith("score.") and ("attn_module.score.weight" in self.model.state_dict()):
                    remapped["attn_module." + k] = v
                elif k.startswith("attn_module.") and ("score.weight" in self.model.state_dict()):
                    remapped[k.replace("attn_module.", "", 1)] = v
                else:
                    remapped[k] = v
            weights = remapped
