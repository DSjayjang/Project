...
#         means = np.nanmean(X_copy)
#         stds = np.nanstd(X_copy)

#         for i in range(0, X_copy.shape[0]):
#             if stds == 0 or np.isnan(stds) or np.isnan(X_copy[i]):
#                 X_copy[i] = 0
#             else:
#                 X_copy[i] = (X_copy[i] - means) / stds
#     else:
#         means = np.nanmean(X_copy, axis=0)
#         stds = np.nanstd(X_copy, axis=0)

#         for i in range(0, X_copy.shape[0]):
#             for j in range(0, X_copy.shape[1]):
#                 if np.isnan(X_copy[i,j]):
#                     X_copy[i, j] = means[j]
#                 else:
#                     X_copy[i, j] = (X_copy[i, j] - means[j]) / stds[j]
...
def Z_Score_new(X):
    ...
    if len(X_copy.shape) == 1:
        ...
        for i in range(0, X_copy.shape[0]):
            for j in range(0, X_copy.shape[1]):
                if stds[j] == 0 or np.isnan(stds[j]) or np.isnan(X_copy[i,j]):
                    X_copy[i, j] = 0
                else:
                    X_copy[i, j] = (X_copy[i, j] - means[j]) / stds[j]
    return X_copy
