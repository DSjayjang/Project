...
def train(model, criterion, optimizer, train_loader, max_epochs, dataset_name, save_model):
    ...
    for epoch in range(max_epochs):
        ...
        for bg, feat_2d, target, scaffold_ids, _ in train_loader:
            ...
            pred, attn_list, h_spec, h_inv = model(bg, feat_2d)
            loss_pred = criterion(pred, target)
            scaf_losses = model.compute_scaffold_losses(
                h_spec=h_spec,
                h_inv=h_inv,
                scaffold_ids=scaffold_ids)

            loss = loss_pred + scaf_losses["total"]
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            ...
            total_n += bs
        train_loss=total_loss_sum / total_n
