...
def descriptor_selection_freesolv(samples):
    ...
    for i in range(0, len(samples)):
        ...
        # 6
        self_feats[i, 5] = mol_graph.NumHDonors
        self_feats[i, 6] = mol_graph.SlogP_VSA2
        self_feats[i, 7] = mol_graph.SlogP_VSA12
        self_feats[i, 8] = mol_graph.HallKierAlpha
        self_feats[i, 9] = mol_graph.EState_VSA10
        # 11
        self_feats[i, 10] = mol_graph.fr_halogen
        self_feats[i, 11] = mol_graph.Chi1n
        self_feats[i, 12] = mol_graph.Chi1
        self_feats[i, 13] = mol_graph.MaxEStateIndex
        self_feats[i, 14] = mol_graph.MaxAbsEStateIndex
        # 16
        self_feats[i, 15] = mol_graph.SMR_VSA3
        self_feats[i, 16] = mol_graph.FractionCSP3
        self_feats[i, 17] = mol_graph.SMR_VSA7
        self_feats[i, 18] = mol_graph.MinPartialCharge
        self_feats[i, 19] = mol_graph.HeavyAtomCount
        # 21
        self_feats[i, 20] = mol_graph.VSA_EState3
        self_feats[i, 21] = mol_graph.MinAbsPartialCharge
        self_feats[i, 22] = mol_graph.SlogP_VSA6
        self_feats[i, 23] = mol_graph.Chi3n
        self_feats[i, 24] = mol_graph.VSA_EState10
...
def descriptor_selection_esol(samples):
    self_feats = np.empty((len(samples), 21), dtype=np.float32)
    smiles_list=[]
    ...
    for i in range(0, len(samples)):
        ...
        self_feats[i, 2] = mol_graph.MaxAbsPartialCharge
        self_feats[i, 3] = mol_graph.SlogP_VSA6
        self_feats[i, 4] = mol_graph.HallKierAlpha
        # 6
        self_feats[i, 5] = mol_graph.LabuteASA
        self_feats[i, 6] = mol_graph.fr_Ar_NH
        self_feats[i, 7] = mol_graph.PEOE_VSA6
        self_feats[i, 8] = mol_graph.FpDensityMorgan1
        self_feats[i, 9] = mol_graph.NumAromaticCarbocycles
        # 11
        self_feats[i, 10] = mol_graph.TPSA
        self_feats[i, 11] = mol_graph.fr_benzene
        self_feats[i, 12] = mol_graph.MaxEStateIndex
        self_feats[i, 13] = mol_graph.MaxAbsEStateIndex
        self_feats[i, 14] = mol_graph.fr_NH1
        # 16
        self_feats[i, 15] = mol_graph.EState_VSA5
        self_feats[i, 16] = mol_graph.EState_VSA7
        self_feats[i, 17] = mol_graph.fr_Ar_OH
        self_feats[i, 18] = mol_graph.Chi0
        self_feats[i, 19] = mol_graph.EState_VSA8
        # 21
        self_feats[i, 20] = mol_graph.BertzCT
...
def descriptor_selection_lipo(samples):
    self_feats = np.empty((len(samples), 24), dtype=np.float32)
    smiles_list=[]
    ...
    for i in range(0, len(samples)):
        ...
        self_feats[i, 2] = mol_graph.PEOE_VSA7
        self_feats[i, 3] = mol_graph.Ipc
        self_feats[i, 4] = mol_graph.fr_sulfonamd
        # 6
        ...
        self_feats[i, 9] = mol_graph.fr_unbrch_alkane
        # 11
        self_feats[i, 10] = mol_graph.fr_guanido
        self_feats[i, 11] = mol_graph.PEOE_VSA12
        self_feats[i, 12] = mol_graph.FpDensityMorgan1
        self_feats[i, 13] = mol_graph.fr_HOCCN
        self_feats[i, 14] = mol_graph.NumSaturatedCarbocycles
        # 16
        self_feats[i, 15] = mol_graph.VSA_EState5
        self_feats[i, 16] = mol_graph.fr_sulfide
        self_feats[i, 17] = mol_graph.fr_piperdine
        self_feats[i, 18] = mol_graph.fr_amidine
        self_feats[i, 19] = mol_graph.BalabanJ
        # 21
        self_feats[i, 20] = mol_graph.NHOHCount
        self_feats[i, 21] = mol_graph.NumAromaticHeterocycles
        self_feats[i, 22] = mol_graph.NumAromaticRings
