...
class BilinearAttention(nn.Module):
    ...
    def forward(self, X: torch.Tensor, Y: torch.Tensor):
        ...
        YV_attn   = self.V_attn(Y)    # (bs, M, K) 렐루 제거
...
class Net(nn.Module):
    def __init__(self, dim_in: int, dim_desc_2d: int):
        super().__init__()
        self.dim_in = dim_in
