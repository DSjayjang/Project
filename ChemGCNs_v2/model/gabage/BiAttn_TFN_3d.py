...
class BiAttn_TFN_hg_3desc_Net(nn.Module):
    def __init__(self, dim_in, dim_2d_desc, dim_3d_desc):
        super(BiAttn_TFN_hg_3desc_Net, self).__init__()
    ...
    def forward(self, g, desc_2d, desc_3d):
        ...
        h_d = self.proj_3d(desc_3d)    # (B, d_h)
        ...
        desc_3d_gate = a * desc_3d      # (B, dim_2d_desc)
        ...
        fusion = torch.bmm(hg.unsqueeze(2), desc_3d_gate.unsqueeze(1)).view(batch_size, -1)
        # MLP
...
# 2nd
class BiAttn_TFN_hg_3dgated_Net(nn.Module):
    def __init__(self, dim_in, dim_2d_desc, dim_3d_desc):
        super(BiAttn_TFN_hg_3dgated_Net, self).__init__()
    ...
    def forward(self, g, desc_2d, desc_3d):
        ...
        gate_vec = torch.sigmoid((h_g @ self.W3) * h_d)   # (B, d_h)
        # bilinear-attended descriptor vector
        v3 = gate_vec * h_d                                # (B, d_h)
...
# 3rd
class BiAttn_TFN_hgg_3desc_Net(nn.Module):
    def __init__(self, dim_in, dim_2d_desc, dim_3d_desc):
        super(BiAttn_TFN_hgg_3desc_Net, self).__init__()
        ...
        # MLP head
        # self.fc1 = nn.Linear(d_g + dim_2d_desc, mlp_hidden1)
        self.fc1 = nn.Linear((d_h+1) * (dim_3d_desc+1), mlp_hidden1)
        self.fc2 = nn.Linear(mlp_hidden1, mlp_hidden2)
        self.fc3 = nn.Linear(mlp_hidden2, dim_out)
    ...
    def forward(self, g, desc_2d, desc_3d):
        ...
        fusion = torch.bmm(h_g.unsqueeze(2), desc_3d_gate.unsqueeze(1)).view(batch_size, -1)
        # MLP
        out = F.relu(self.bn1(self.fc1(fusion)))
        out = self.dropout(out)
        out = F.relu(self.bn2(self.fc2(out)))
...
# 4th
class BiAttn_TFN_hgg_3dgated_Net(nn.Module):
    def __init__(self, dim_in, dim_2d_desc, dim_3d_desc):
        super(BiAttn_TFN_hgg_3dgated_Net, self).__init__()
    ...
    def forward(self, g, desc_2d, desc_3d):
        ...
        fusion = torch.bmm(h_g.unsqueeze(2), v3.unsqueeze(1)).view(batch_size, -1)
        # MLP
        out = F.relu(self.bn1(self.fc1(fusion)))
        out = self.dropout(out)
        out = F.relu(self.bn2(self.fc2(out)))
