...
# Cross Attention + Tensor Fusion
class Net_2d(nn.Module):
    def __init__(self, dim_in, dim_2d_desc, dim_3d_desc, d_t: int=64, d_k: int=64):
        super(Net_2d, self).__init__()
        ...
        # MLP blocks
        self.fc1 = nn.Linear((self.dim_graph_emb+1) * (self.d_k+1), dim_out_fc1)
        # self.fc1 = nn.Linear(self.d_k*2, dim_out_fc1)
        self.fc2 = nn.Linear(dim_out_fc1, dim_out_fc2)
        self.fc3 = nn.Linear(dim_out_fc2, dim_out)
...
class MultiHeadCrossAttn(nn.Module):
    ...
    def forward(self, hg: torch.Tensor, desc: torch.Tensor):
        ...
        Q = Q.view(B, self.H, self.d_h).unsqueeze(2)             # (B,H,1,d_h)
        K = K.view(B, D, self.H, self.d_h).transpose(1, 2)       # (B,H,D,d_h)
        V = V.view(B, D, self.H, self.d_h).transpose(1, 2)       # (B,H,D,d_h)
...
class Net_2d_grid(nn.Module):
    def __init__(self, dim_in, dim_2d_desc, dim_3d_desc, d_t, d_k, dim_out_fc1, dim_out_fc2, drop_out, num_heads=4):
        super().__init__()
        self.dim_graph_emb = 20
        self.dim_2d_desc = dim_2d_desc
        self.dim_3d_desc = dim_3d_desc
        ...
        self.attn_2d = CrossAttn(d_g=self.dim_graph_emb, d_desc=dim_2d_desc, d_t=self.d_t, d_k=self.d_k)
        self.attn_3d = CrossAttn(d_g=self.dim_graph_emb, d_desc=dim_3d_desc, d_t=self.d_t, d_k=self.d_k)
        # self.attn_2d = MultiHeadCrossAttn(d_g=self.dim_graph_emb, d_desc=dim_2d_desc, d_t=self.d_t, d_k=self.d_k, num_heads=self.num_heads)
        # self.attn_3d = MultiHeadCrossAttn(d_g=self.dim_graph_emb, d_desc=dim_3d_desc, d_t=self.d_t, d_k=self.d_k, num_heads=self.num_heads)
