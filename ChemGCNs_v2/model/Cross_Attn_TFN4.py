...
class Net(nn.Module):
    ...
    def forward(self, g, desc_2d, desc_3d, return_attn=False):
        ...
        hg2, attn3d, z3d = self.attn_3d(hg1, desc_3d) # hg: (bs, d_g), desc_3d: (bs, dim_3d_desc)

        # Tensor Fusion
        ones = torch.ones(batch_size, 1, device=hg.device, dtype=hg.dtype)

        hg = torch.cat((hg, ones), dim = 1)
        z2d = torch.cat((z2d, ones), dim = 1)
        z3d = torch.cat((z3d, ones), dim = 1)

        # tensor fusion
        fusion_tensor = torch.einsum('bi, bj, bk -> bijk', hg, z2d, z3d) # (bs, 21, 21, 21)
