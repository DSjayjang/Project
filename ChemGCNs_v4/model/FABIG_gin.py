...
class GINLayer(nn.Module):
    def __init__(self, dim_in, dim_out, dropout: float = 0.1):
        super().__init__()
        # 원논문 표준: BN 포함 2-layer MLP
        mlp = nn.Sequential(
        ...
        self.conv = GINConv(
            ...
            aggregator_type='sum',
            init_eps=0.0,
            learn_eps=True   # ε 학습 가능 — 원논문 GIN-ε
        )
        # dim이 다를 때를 위한 projection
        self.residual = nn.Linear(dim_in, dim_out) if dim_in != dim_out else nn.Identity()

    def forward(self, g, feat):
        # return self.conv(g, feat)  # 활성화 포함되어 있으므로 밖에서 ReLU 불필요
        return self.conv(g, feat)  + self.residual(feat)# 활성화 포함되어 있으므로 밖에서 ReLU 불필요
...
class FamilyBilinearAttention(nn.Module):
    ...
    def forward(self, tokns_k, H_pad, mask):
        ...
        with torch.no_grad():
            ...
            score_std = active_scores.std().item()
            score_range = (active_scores.max() - active_scores.min()).item()
            # print(f"  score std={score_std:.5f}, range={score_range:.5f}")
            # std < 0.01 이면 attention이 사실상 uniform
        # ────────────────────────────────────────────────────
...
class Net(nn.Module):
    def __init__(self, dim_in: int, dim_feat_2d: int):
        ...
        # ── GCN ──────────────────────────────────────────
        self.gc1 = GINLayer(dim_in, self.dim_graph_hidden)
        self.gc2 = GINLayer(self.dim_graph_hidden, self.dim_graph)
    ...
    def forward(self, g, desc):
        ...
        """
        # ── GCN ──────────────────────────────────────────
        h = self.gc1(g, g.ndata['feat'])
        h = self.gc2(g, h)                # [N_total, dim_graph] (N_total: 각 배치당 총 노드 개수)
        # h = self.gc2(g, h)                # [N_total, dim_graph] (N_total: 각 배치당 총 노드 개수)
        g.ndata['h'] = h

        hg = dgl.sum_nodes(g, 'h')               # [bs, dim_graph]
