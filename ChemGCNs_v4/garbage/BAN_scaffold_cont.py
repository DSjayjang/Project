...
def grl(x, lambd: float = 1.0):
    return GradReverse.apply(x, lambd)


class ContrastiveScaffoldHead(nn.Module):
    ...
    def forward(self, z):
        return F.normalize(self.proj(z), dim=-1)  # (bs, dim_out)
...
class BilinearAttention(nn.Module):
    ...
    def forward(self, X: torch.Tensor, Y: torch.Tensor):
        ...
        for g in range(self.glimpse):
            ...
            # new
            # attn = self.attn_drop(attn)               # Attention Dropout 추가
...
class Net(nn.Module):
    def __init__(self, dim_in: int, dim_desc_2d: int, n_scaffolds_train, grl_lambda: float = 0.2):
        super().__init__()
        self.dim_in = dim_in
        ...
        # # scaffold classifier head (아주 가볍게)
        # self.grl_lambda = 0.5
        # self.scaffold_head = None
        # if n_scaffolds_train is not None:
        #     self.scaffold_head = nn.Sequential(
        #         nn.Linear(self.dim_graph, self.dim_graph),
        #         nn.ReLU(),
        #         nn.Linear(self.dim_graph, n_scaffolds_train)
        #     )

        # ✅ contrastive scaffold head (없으면 None)
        self.grl_lambda = 64
        self.temperature = 0.1
        self.scaffold_head = None
        if n_scaffolds_train is not None:
            self.scaffold_head = ContrastiveScaffoldHead(self.dim_graph, self.temperature)
    ...
    def forward(self, g, desc_2d):
        ...
        scaffold_proj = None
        if self.scaffold_head is not None:
            z_grl = grl(fused, self.grl_lambda)
            scaffold_proj = self.scaffold_head(z_grl)  # (bs, scaf_proj_dim)
        ...
        return out, attn_list, scaffold_proj # 추가
