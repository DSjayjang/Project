...
# 2. train index: SMILES와 target이 모두 일치하는 행
train_idx = df.index[df.apply(lambda row: train_dict.get(row['smiles'], None) == y_ISIS.loc[row.name], axis=1)]

# 3. test index = 전체 index - train index
test_idx = X_ISIS.index.difference(train_idx)

# 4. 최종 분할
X_train = X_ISIS.loc[train_idx]
y_train = y_ISIS.loc[train_idx]

X_test = X_ISIS.loc[test_idx]
y_test = y_ISIS.loc[test_idx]
