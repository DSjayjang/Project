...
import utils.mol_conv_scaffold_bilinear as mc
from utils import trainer_scaffold_bilinear2
...
def main():
    ...
    if backbone == 'GCN':
        ...
        model_Fusion = Bilinear_Attn.Net_New(dim_atomic_feat, 1, num_descriptors).to(device)
    ...
    print('--------- Bilinear kronecker-product with descriptor selection ---------')
    test_losses['Backbone_Fusion'] = trainer_scaffold_bilinear2.cross_validation(dataset, model_Fusion, criterion, folds, K, BATCH_SIZE, MAX_EPOCHS, trainer_scaffold_bilinear2.train_model, trainer_scaffold_bilinear2.test_model, descriptors)
