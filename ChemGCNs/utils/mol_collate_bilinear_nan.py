...
# Self-Curated Gas
def descriptor_selection_scgas_clear(samples):
    ...
    for i in range(0, len(samples)):
        ...
        self_feats[i, 9] = mol_graph.NumRadicalElectrons
        # 11
        self_feats[i, 10] = mol_graph.fr_pyridine
        self_feats[i, 11] = mol_graph.fr_quatN
        self_feats[i, 12] = mol_graph.fr_sulfide
        self_feats[i, 13] = mol_graph.fr_sulfonamd
        self_feats[i, 14] = mol_graph.FpDensityMorgan1
        # 16
        self_feats[i, 15] = mol_graph.FpDensityMorgan2
        self_feats[i, 16] = mol_graph.FpDensityMorgan3
        self_feats[i, 17] = mol_graph.fr_sulfone
        self_feats[i, 18] = mol_graph.fr_term_acetylene
        self_feats[i, 19] = mol_graph.fr_tetrazole

        self_feats[i, 20] = mol_graph.fr_thiazole
        self_feats[i, 21] = mol_graph.fr_thiocyan
        self_feats[i, 22] = mol_graph.fr_thiophene
        self_feats[i, 23] = mol_graph.fr_unbrch_alkane
        self_feats[i, 24] = mol_graph.fr_urea
...
# def descriptor_selection_scgas_debug(samples):
#     num_descriptors = 208
#     self_feats = np.empty((len(samples), num_descriptors), dtype=np.float32)
#     failed_features = set()
...
#         ]):
#             try:
#                 self_feats[i, j] = getattr(mol_graph, attr)
#                 # print(attr)
#             except Exception as e:
#                 print(e)
#                 # self_feats[i, j] = np.nan  # 또는 0으로 대체 가능
#                 failed_features.add(attr)

#     if failed_features:
#         print("❌ Failed features:")
#         for f in sorted(failed_features):
#             print(f" - {f}")
