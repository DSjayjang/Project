...
def descriptor_selection_7(samples):
    ...
    for i in range(0, len(samples)):
        ...
        self_feats[i, 4] = mol_graph.PEOE_VSA6
        # 6
        self_feats[i, 5] = mol_graph.HeavyAtomMolWt
...
def descriptor_selection_10(samples):
    ...
    for i in range(0, len(samples)):
        ...
        self_feats[i, 8] = mol_graph.NOCount
...
def descriptor_selection_20(samples):
    ...
    for i in range(0, len(samples)):
        ...
        self_feats[i, 3] = mol_graph.fr_benzene
        self_feats[i, 4] = mol_graph.VSA_EState6
        # 6
        self_feats[i, 5] = mol_graph.SlogP_VSA6
        self_feats[i, 6] = mol_graph.MolLogP
        self_feats[i, 7] = mol_graph.fr_quatN
        self_feats[i, 8] = mol_graph.HeavyAtomMolWt
        self_feats[i, 9] = mol_graph.EState_VSA2
        # 11
        self_feats[i, 10] = mol_graph.SMR_VSA10
        self_feats[i, 11] = mol_graph.NumHDonors
        self_feats[i, 12] = mol_graph.SlogP_VSA3
        self_feats[i, 13] = mol_graph.BertzCT
        self_feats[i, 14] = mol_graph.MaxAbsPartialCharge
        # 16
        self_feats[i, 15] = mol_graph.Chi3n
        self_feats[i, 16] = mol_graph.BCUT2D_LOGPHI
        self_feats[i, 17] = mol_graph.NHOHCount
        self_feats[i, 18] = mol_graph.PEOE_VSA7
        self_feats[i, 19] = mol_graph.SlogP_VSA1
