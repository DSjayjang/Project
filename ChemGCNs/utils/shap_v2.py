...
class SHAP:
    ...
    def _define_model_wrapper(self):
        ...
        def model_wrapper(x_concat):
            ...
            # Kronecker (outer) product: (N, g, 1) @ (N, 1, D) = (N, g, D)
            fused = torch.bmm(z.unsqueeze(2), d.unsqueeze(1))
            fused = fused.view(fused.size(0), -1)  # (N, g*D)
    ...
    # -------------------------------------------------
    def plot_summary(self, save_path=f'./results_figure/shap/shap_{DATASET_NAME}_{MAX_EPOCHS}_{K}_kronecker.png', max_display=10):
        """
