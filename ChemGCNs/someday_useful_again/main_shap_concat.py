...
from model import CONCAT_DS_shap
from configs import config
from configs.config import SET_SEED, DATASET_NAME, DATASET_PATH, BATCH_SIZE, MAX_EPOCHS, K

from someday_useful_again.shap_v2_concat import SHAP # SHAP 분석
...
def main():
    ...
    model_concat_ds = CONCAT_DS_shap.concat_Net(dim_atomic_feat, 1, num_descriptors).to(device)
    ...
    # evaluation
    print('concatenation fusion with descriptor selection')
    val_losses['concat_ds'], best_model, best_k = evaluation.cross_validation(train_dataset, model_concat_ds, criterion, K, BATCH_SIZE, MAX_EPOCHS, evaluation.train_model, evaluation.val_model, descriptors)
    print('Val loss (concat_ds): ' + str(val_losses['concat_ds']))
